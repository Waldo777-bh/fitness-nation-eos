...
export default function RocksPage() {
  ...
  async function addRock() {
    ...
    setTitle(''); setDescription(''); setOwnerId(''); setShowForm(false);
    load();
  }
  ...
  return (
    <>
      <PageHeader title="Rocks" subtitle={`${quarter?.label ?? ''} quarterly priorities`}>
        <button className="btn" onClick={() => setShowForm((s) => !s)}>+ Add Rock</button>
      ...
      {showForm && (
        <div className="panel p-5 mb-5 flex flex-col gap-3">
          ...
          <textarea className="input" placeholder="Description / definition of done" value={description} onChange={(e) => setDescription(e.target.value)} />
          <div className="flex gap-3">
            <select className="input !w-auto" value={ownerId} onChange={(e) => setOwnerId(e.target.value)}>
              <option value="">Owner…</option>
              {team.map((t) => <option key={t.id} value={t.id}>{t.name}</option>)}
            </select>
            <button className="btn" onClick={addRock}>Save</button>
          </div>
        </div>
      )}
        ...
        {rocks.map((r) => {
          ...
          return (
            <div key={r.id} className="panel p-5">
              ...
              {r.description && <p className="text-sm text-zinc-400 mb-3">{r.description}</p>}
